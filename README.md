...
from asyncio.staggered import staggered_race  # This import seems unused and may cause errors
import pandas as pd
...
def load_overall_analysis():
    ...
    ax4.bar(temp_df['x_axis'], temp_df['amount'])
    st.pyplot(fig4)
...
def load_startup_details(startup):
    ...
    if not startup_df.empty:
        # Most recent funding details
        st.subheader('Most Recent Funding Details')
        recent_funding = startup_df.sort_values('date', ascending=False).head(1)
        st.dataframe(recent_funding[['date', 'amount', 'round', 'investors', 'vertical', 'city']])

        # All funding rounds
        st.subheader('All Funding Rounds')
        st.dataframe(startup_df[['date', 'amount', 'round', 'investors']].sort_values('date', ascending=False))

        # Total funding amount
        total_funding = startup_df['amount'].sum()
        st.metric('Total Funding', f"{round(total_funding)} Cr")
...
if option == 'Overall Analysis':
    ...
    # Correctly split and flatten the investors list
    investors_list = []
    for investors_str in df['investors'].dropna():
        if isinstance(investors_str, str):
            investors_list.extend([investor.strip() for investor in investors_str.split(',')])

    selected_investor = st.sidebar.selectbox('Select Investor', sorted(set(investors_list)))
